...
# Legacy keys (P2PKH)
EX_MAIN_PRIVATE = codecs.decode('0488ade4', 'hex') # xprv
EX_MAIN_PUBLIC  = codecs.decode('0488b21e', 'hex') # xpub
EX_TEST_PRIVATE = codecs.decode('04358394', 'hex') # tprv
EX_TEST_PUBLIC  = codecs.decode('043587CF', 'hex') # tpub
# Compatibility keys (P2SH)
Y_MAIN_PRIVATE = codecs.decode('049d7878', 'hex') # yprv
Y_MAIN_PUBLIC  = codecs.decode('049d7cb2', 'hex') # ypub
Y_TEST_PRIVATE = codecs.decode('044a4e28', 'hex') # uprv
Y_TEST_PUBLIC  = codecs.decode('044a5262', 'hex') # upub
# segwit keys (Bech32)
Z_MAIN_PRIVATE = codecs.decode('04b2430c', 'hex') # zprv
Z_MAIN_PUBLIC  = codecs.decode('04b24746', 'hex') # zpub
Z_TEST_PRIVATE = codecs.decode('045f18bc', 'hex') # vprv
Z_TEST_PUBLIC  = codecs.decode('045f1cf6', 'hex') # vpub
...
class BIP32Key(object):
    ...
    def fromEntropy(entropy, public=False, testnet=False):
        ...
        key = BIP32Key(secret=Il, chain=Ir, depth=0, index=0, fpr=b'\0\0\0\0', public=False, testnet=testnet, bip=32)
        if public:
            key.SetPublic()
        return key
    ...
    def fromExtendedKey(xkey, public=False):
        ...
        if version == EX_MAIN_PRIVATE:
            ...
            is_testnet = True
            is_pubkey = True
        elif version == Y_MAIN_PRIVATE:
            bip = 49
            is_testnet = False
            is_pubkey = False
        elif version == Y_TEST_PRIVATE:
            bip = 49
            is_testnet = True
            is_pubkey = False
        elif version == Y_MAIN_PUBLIC:
            bip = 49
            is_testnet = False
            is_pubkey = True
        elif version == Y_TEST_PUBLIC:
            bip = 49
            is_testnet = True
            is_pubkey = True
        elif version == Z_MAIN_PRIVATE:
            bip = 84
            is_testnet = False
            is_pubkey = False
        elif version == Z_TEST_PRIVATE:
            bip = 84
            is_testnet = True
            is_pubkey = False
        elif version == Z_MAIN_PUBLIC:
            bip = 84
            is_testnet = False
            is_pubkey = True
        elif version == Z_TEST_PUBLIC:
            bip = 84
            is_testnet = True
            is_pubkey = True
        else:
        ...
        key = BIP32Key(secret=secret, chain=chain, depth=depth, index=child, fpr=fpr, public=is_pubkey, testnet=is_testnet, bip=bip)
    ...
    def __init__(self, secret, chain, depth, index, fpr, public=False, testnet=False, bip=32):
        """
    ...
    def CKDpriv(self, i):
        ...
        return BIP32Key(secret=secret, chain=Ir, depth=self.depth+1, index=i, fpr=self.Fingerprint(), public=False, testnet=self.testnet, bip=self.bip)
    ...
    def CKDpub(self, i):
        ...
        # Construct and return a new BIP32Key
        return BIP32Key(secret=K_i, chain=Ir, depth=self.depth+1, index=i, fpr=self.Fingerprint(), public=True, testnet=self.testnet, bip=self.bip)
    ...
    def Address(self):
        ...
        if self.bip == 84:
            ...
            return bech32.encode('bc' if not self.testnet else 'tb', 0, keyhash)
        elif self.bip == 49:
            keyhash = hashlib.new('ripemd160', sha256(self.PublicKey()).digest()).digest()
            ...
            addressversion = PREFIX_MAINNET_PKH if not self.testnet else PREFIX_TESTNET_PKH
    ...
    def ExtendedKey(self, private=True, encoded=True):
        ...
        if not self.testnet:
            if self.bip == 84:
                version = Z_MAIN_PRIVATE if private else Z_MAIN_PUBLIC
            elif self.bip == 49:
                version = Y_MAIN_PRIVATE if private else Y_MAIN_PUBLIC
            else:
                version = EX_MAIN_PRIVATE if private else EX_MAIN_PUBLIC
        else:
            if self.bip == 84:
                version = Z_TEST_PRIVATE if private else Z_TEST_PUBLIC
            elif self.bip == 49:
                version = Y_TEST_PRIVATE if private else Y_TEST_PUBLIC
            else:
